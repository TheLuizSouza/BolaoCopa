...
function App() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      // Alimenta o estado 'guesses' com consistência total de dados
      const savedGuesses = {};
  ...
  const handleSaveGuess = async (matchId) => {
    ...
    if (!matchGuess || matchGuess.guess_a === undefined || matchGuess.guess_b === undefined) {
  ...
  // CORRIGIDO: Agora aponta para o endpoint correto que processa os pontos por POST
  const handleFinishMatch = async (match) => {
    ...
    const confirmar = window.confirm(`Encerrar jogo com o placar ${score.goals_a}x${score.goals_b}? Isso irá computar os pontos de todos os usuários.`);
    if (!confirmar) return;
      ...
      <main className="w-full max-w-4xl flex justify-center">
        {!user ? (
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* ABAS DE CONTEÚDO */}
            <div className="md:col-span-2w-full">
              {activeTab === 'palpites' ? (
                    ...
                    matches.map((match) => (
                      <div key={match.id} className="bg-slate-800 p-5 rounded-lg shadow-md border border-slate-700 flex flex-col items-center justify-between gap-4 w-200">
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4 w-full">
                          ...
                          <div className="flex items-center gap-3 font-semibold text-lg justify-center flex-1 w-200 max-w-xl">
                            <div className="flex items-center gap-2 flex-1 justify-end min-w-[120px]">
                              <span className="truncate text-right text-base">{match.team_a_name}</span>
                            ...
                            <div className="flex items-center gap-2 flex-1 justify-start min-w-[120px]">
                              ...
                              <span className="truncate text-left text-base">{match.team_b_name}</span>
                            </div>
                          </div>
                        ...
                        {match.status === 'FINISHED' && (
                          <div className="flex flex-wrap items-center justify-center gap-2 mt-1 border-t border-slate-700/60 pt-3 w-200">
                            ...
                            <div className={`text-xs font-bold px-3 py-1 rounded border shadow-inner ${
                              (match.points_earned ?? 0) === 25
                                ? 'bg-green-950/40 border-green-800 text-green-400' // 25 pontos: Verde
                                : (match.points_earned ?? 0) === 10
                                ? 'bg-blue-950/40 border-blue-800 text-blue-400'  // 10 pontos: Azul
                                : 'bg-red-950/40 border-red-800 text-red-400'      // 0 pontos: Vermelho
                            }`}>
                ...
                <div className="space-y-8 w-full">
                  <div className="bg-slate-800 p-6 rounded-lg shadow-xl border border-red-950 space-y-6 w-full">
                  ...
                  <div className="bg-slate-800 p-6 rounded-lg shadow-xl border border-slate-700 space-y-4 w-full">
                    ...
                    {matches.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {matches.map((match) => (
                          <div key={match.id} className="p-4 bg-slate-900/60 rounded border border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-2">
                              {match.status === 'FINISHED' ? (
                                <span className="text-xs bg-green-900/40 text-green-400 border border-green-800 px-2 py-1 rounded font-bold">Encerrado ({match.goals_a}x{match.goals_b})</span>
                              ) : (
